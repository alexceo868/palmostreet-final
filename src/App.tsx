...
import { Camera, Warehouse, Zap, Gauge, DollarSign, X, Shield, Award, Car, ScanLine, Maximize2, Minimize2, LogOut, Lock, Mail, ChevronRight, User, Users, CheckCircle, Target, TrendingUp, Activity, Star, Copy, Key, Settings, Image as ImageIcon, Upload } from 'lucide-react';
...
// --- PROFILE WIZARD (No Stock Images) ---
const ProfileWizard = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const handleProfileSetup = async () => {
    if (!nickname || !avatar) return; // Obbligatorio avere foto
    setLoading(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-slate-900 border border-white/10 p-6 rounded-2xl w-full max-w-sm">
        ...
        <div className="mb-6">
          <label className="text-xs uppercase text-zinc-500 font-bold mb-2 block">Gemini API Key</label>
          <input 
            type="password" 
            value={key}
          ...
          />
          <p className="text-[10px] text-zinc-500 mt-2">Necessaria per il riconoscimento AI reale. <a href="https://aistudio.google.com/app/apikey" target="_blank" className="text-red-400 underline">Ottieni qui</a>.</p>
        </div>

        <div className="flex space-x-3">
          <button onClick={() => {onSaveKey(key); onClose();}} className="flex-1 bg-red-600 hover:bg-red-500 py-3 rounded-xl font-bold text-white uppercase text-xs tracking-widest">SALVA</button>
...
export default function PalmostreetApp() {
  ...
  useEffect(() => {
    ...
    // Tenta fullscreen automatico (su mobile spesso richiede interazione, ma proviamo)
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (!aiResult) {
            ...
            aiResult = {
                brand: "Simulazione", model: "Auto Demo", year: 2024, hp: 200, value_eur: 30000, list_value: 35000,
                description: "Modalità simulazione attiva. Configura la API Key nelle Impostazioni per l'AI reale.",
                scores: { speed: 3, versatility: 4, quality_price: 5, durability: 4 },
                isSimulation: true
            };
        }
  ...
  const addFriend = async (friendId: string) => {
     ...
     await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid), { friends: updatedFriends });
  };
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-slate-950 text-slate-200 font-exo selection:bg-red-500/30 overflow-x-hidden pb-24 relative">
      <style>{`
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 animate-in slide-in-from-bottom duration-300">
            ...
            <div className={`relative w-full max-w-lg bg-gradient-to-b ${RARITY_CONFIG[selectedCar.rarity].gradient} rounded-t-3xl sm:rounded-3xl overflow-hidden border-t-2 sm:border-2 ${RARITY_CONFIG[selectedCar.rarity].border} shadow-2xl h-[90vh] sm:h-auto overflow-y-auto`}>
                <div className="relative h-64 w-full bg-black group">
                    <img src={selectedCar.imageUrl} className="w-full h-full object-cover opacity-90 group-hover:scale-105 transition-transform duration-700" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent"></div>
                    <button onClick={() => setSelectedCar(null)} className="absolute top-4 right-4 p-2 bg-black/50 rounded-full hover:bg-red-600 transition-colors z-10"><X size={24} className="text-white" /></button>
                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                        <div>
                            <span className={`px-3 py-1 rounded text-xs font-bold uppercase tracking-wider bg-black/80 border ${RARITY_CONFIG[selectedCar.rarity].color} ${RARITY_CONFIG[selectedCar.rarity].border} mb-2 inline-block`}>{selectedCar.rarity}</span>
                            <h2 className="text-3xl font-orbitron font-black text-white italic uppercase leading-none">{selectedCar.model}</h2>
                            <p className="text-zinc-400 font-bold uppercase tracking-widest">{selectedCar.brand}</p>
                        </div>
                    </div>
                </div>
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                        <h3 className="font-orbitron text-sm text-zinc-400 uppercase tracking-widest border-b border-white/10 pb-1">Performance Index</h3>
                        {selectedCar.scores && Object.entries(selectedCar.scores).map(([key, score]) => (
                            <div key={key} className="flex items-center justify-between"><span className="text-xs uppercase text-zinc-300 font-bold w-24">{(key as string).replace('_', ' ')}</span><div className="flex-1 h-2 bg-black rounded-full mx-3 overflow-hidden"><div className={`h-full ${(score as number) >= 4 ? 'bg-green-500' : (score as number) >= 3 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{width: `${((score as number)/5)*100}%`}}></div></div><span className="text-xs font-mono font-bold w-6 text-right">{score as number}/5</span></div>
                        ))}
                    </div>
      ...
      <main className="pt-20 px-4">
        {view === 'garage' && (
            <div className="space-y-6 animate-in slide-in-from-bottom-4">
                <div className="flex justify-between items-end"><h2 className="text-2xl font-orbitron font-bold text-white">GARAGE</h2><span className="text-xs font-mono text-zinc-500">{cars.length} AUTO</span></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {cars.map(car => (
                        <div key={car.id} onClick={() => setSelectedCar(car)} className="perspective-card group cursor-pointer">
                            <div className={`relative bg-slate-900 border-2 ${RARITY_CONFIG[car.rarity].border} rounded-xl overflow-hidden transform transition-transform duration-300 group-hover:rotate-x-2 group-hover:scale-[1.02] shadow-2xl`}>
                                <div className="h-48 w-full bg-black relative overflow-hidden">
                                    ...
                                    <img src={car.imageUrl} className="w-full h-full object-contain mix-blend-normal z-10 relative drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)]" />
                                ...
                                <div className="p-4 bg-slate-900 relative z-20">
                                    <div className="flex justify-between items-start mb-2"><div><h3 className="font-orbitron font-bold text-white text-lg leading-none">{car.model}</h3><span className="text-[10px] uppercase text-zinc-500 tracking-wider">{car.brand}</span></div><span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${RARITY_CONFIG[car.rarity].color} ${RARITY_CONFIG[car.rarity].border}`}>{car.rarity}</span></div>
                                    <div className="w-full h-[1px] bg-white/10 mb-2"></div>
                                    <div className="flex justify-between text-xs font-mono text-zinc-400"><span>{car.year}</span><span>{car.hp} HP</span></div>
                                </div>
                            </div>
                        </div>
                    ))}
                    {cars.length === 0 && (<div className="col-span-full py-20 text-center opacity-50"><Warehouse size={48} className="mx-auto mb-4 text-zinc-600" /><p>IL TUO GARAGE È VUOTO</p><p className="text-xs mt-2">Usa lo scanner per iniziare</p></div>)}
                </div>
            </div>
        )}
        ...
        {view === 'social' && (
            <div className="space-y-8 animate-in slide-in-from-right">
                <div>
                    ...
                    <div className="space-y-3">
                        {objectives.map(obj => (
                            <div key={obj.id} className={`p-4 rounded-xl border border-white/10 bg-gradient-to-r ${RARITY_CONFIG[obj.rarity].bg} flex justify-between items-center`}><div><div className="font-bold text-sm text-white">{obj.text}</div><div className="text-[10px] text-zinc-400 mt-1 flex space-x-3"><span className="text-blue-400 font-bold">+{obj.xp} XP</span></div></div><div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${obj.done ? 'bg-green-500 border-green-500' : 'border-zinc-600'}`}>{obj.done && <CheckCircle size={14} className="text-black" />}</div></div>
                        ))}
                    </div>
                </div>
                <div>
                    ...
                    {friends.length === 0 ? (<div className="text-center p-6 border border-dashed border-white/10 rounded-xl text-zinc-500 text-xs">Nessun amico seguito</div>) : (<div className="space-y-2">{friends.map((f, i) => (<div key={i} className="flex items-center justify-between p-3 bg-slate-900 rounded-lg border border-white/5"><div className="flex items-center space-x-3"><div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center"><User size={14} /></div><span className="text-sm font-bold">ID: {f.id.slice(0,6)}...</span></div><span className="text-[10px] text-zinc-500">AMICO</span></div>))}</div>)}
                </div>
            </div>
        )}
      </main>
